...
当前分支已经落地三协议 raw record/replay、跨协议 scenario 编译、Vue 3 + Varlet 管理后台、AI SDK provider 探针、原子运行时配置和 JSONL capture 管理。实际控制面如下：
...
- Record 模式只允许 `raw` transport；AI SDK 会归一化事件，不能生成 body-exact capture。
- AI SDK 7 用于 OpenAI Chat、OpenAI Responses、Anthropic Messages 的显式 provider 探针；探针 key 只存在于请求内存，最多请求 8 个输出 token。
...
AI SDK 会解析和归一化请求、响应及流事件，因此不能放在透明代理的必经路径上。当前实现只用 AI SDK 发起三类 provider 的最小语义探针；同协议录制与透传使用 Node.js 低层 HTTP transport。跨协议重放由项目自己的 scenario IR/compiler 完成，不把 AI SDK 探针描述成尚不存在的 semantic transport。
...
10. 接入 AI SDK 7，但不牺牲 raw passthrough fidelity。
...
部分 Anthropic/AI SDK 场景使用 `Authorization: Bearer <token>`。Raw gateway 不猜测鉴权方案，而是透传全部 end-to-end headers。录制副本只脱敏 `x-api-key`、`Authorization`、cookie 等 credential；`anthropic-version` 和 `anthropic-beta` 必须原值保存。
...
1. `@ai-sdk/openai`：配置 custom `baseURL`，分别使用 `.chat()` 和 `.responses()` 做显式 capability probe。
2. `@ai-sdk/anthropic`：配置 custom `baseURL`，使用 `.messages()` 做显式 capability probe。
3. `ai.generateText()` 把探针输出限制为 8 token；该动作可能计费，UI 必须明确提示。
4. raw record path 永远绕过 AI SDK；否则无法保存原始 SSE bytes、未知 event、JSON 表达形式和 Chat `[DONE]`。

实现以 [AI SDK introduction](https://ai-sdk.dev/docs/introduction)、[OpenAI provider](https://ai-sdk.dev/providers/ai-sdk-providers/openai) 和 [Anthropic provider](https://ai-sdk.dev/providers/ai-sdk-providers/anthropic) 的当前官方说明为准。

Provider 按探针请求创建，只在内存中接收临时 key，不缓存、不记录。探针使用同一套 SSRF policy 和 DNS pinning。当前 `ai-sdk` 配置值只用于显式探针，不是 Record transport；切换 Record 时服务端会要求所有已启用 endpoint 使用 `raw`。
...
interface CommonUpstream {
  enabled: boolean
  allowPrivateNetwork: boolean
}

interface UpstreamConfig extends CommonUpstream {
  protocol: Protocol
...
Raw transport 接受 `baseUrl`，再按协议只追加一次 canonical endpoint。URL 必须通过 WHATWG URL parse，禁止 userinfo、query 和 fragment。AI SDK probe 使用同一个 `baseUrl`，model 与 key 由用户当次输入，UI 明示可能计费且不会持久化。
...
POST /upstreams/ai-sdk-check
```

`check` 可选：

- raw：只做 URL/DNS/TLS/HTTP capability check；
- AI SDK：执行最小 semantic request，结果明确标记为会产生真实 provider 调用和费用。

后台不得自动发送计费请求；用户必须点击确认。
...
- 三类 upstream cards；
- raw gateway 与 AI SDK probe 共用 provider base URL 配置，但分别使用透明 HTTP transport 与 AI SDK provider transport；
- pass-through auth 状态；
...
- curl、OpenAI SDK、Anthropic SDK、AI SDK 示例片段。
...
- raw transparent proxy 不跟随 redirect，3xx status/headers/body 原样返回；AI SDK probe 也拒绝 redirect；
- local model server 场景由 admin 对单个 upstream 显式打开 `allowPrivateNetwork`；
- capability probe 有 5 秒连接检查超时，AI SDK probe 有 15 秒总超时；
...
AI SDK/provider packages 为 ESM，但这本身不强迫整个仓库迁移。这里选择统一 ESM，是为了兑现“使用最新库”、避免长期维护 CommonJS/ESM 双边界；它必须作为独立 migration gate 完成，不能与 gateway 行为改动混成一次不可定位的重构。
...
| 3 | text/function-tool/basic-usage/finish/error 公共 IR 与三协议 compiler；AI SDK provider probe | extension IR、strict/best-effort、完整 provider semantics |
...
### Phase 3：协议 parser、IR、AI SDK 与转换
...
4. 接入 AI SDK OpenAI/Anthropic provider adapter 与显式 capability check；semantic transport 留作后续扩展。
5. 公共子集之外的 strict/best-effort diagnostics 留作后续扩展。
...
Release compatibility suite 再以锁定版本的 `openai`、`@anthropic-ai/sdk` 和本项目已安装的 AI SDK providers 作为真实 downstream clients，覆盖三个 endpoint 的 stream/non-stream/text/function-call/error。它们只作为 dev contract tests，不进入 gateway 实现或 production dependencies。
...
2. Record 模式不经 JSON parse/re-serialize 或 AI SDK 改写 raw request/response body。
...
13. AI SDK 已实际用于 OpenAI/Anthropic provider adapter capability check，但不进入 raw fidelity path。
...
| AI SDK 位置 | provider capability probe，不在 raw proxy hot path；跨协议由 scenario compiler 完成 |
| 多实例 | 首版 single process / single writer |
| Gemini | 不扩张，仅保持现有兼容 |

按此边界实施，可以同时满足透明录制、可验证重放、协议转换和可视化编辑，又不会用“AI SDK 解析后的流”冒充原始 wire capture。
