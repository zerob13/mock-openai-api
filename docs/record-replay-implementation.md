...
每个协议独立保存一份 upstream 配置，但 Recording 每次只激活其中一个：
...
### Phase 2：Raw replay + built-in replay sources
...
- 无 upstream 时 Replay 仍以 built-in scenario 默认可用。
